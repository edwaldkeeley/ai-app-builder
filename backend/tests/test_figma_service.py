"""Tests for Figma modules: URL parsing, color conversion, node walking,
JSON filtering, canvas detection, prompt building, and cache management.
...
from app.services.figma_tree_walker import build_design_prompt, walk_nodes
...
class TestExtractFileKey:
    def test_standard_file_url(self):
        assert extract_file_key("https://www.figma.com/file/ABC123/My-Design") == "ABC123"

    def test_design_url(self):
        assert extract_file_key("https://www.figma.com/design/ABC123/My-Design") == "ABC123"

    def test_url_with_trailing_slash(self):
        assert extract_file_key("https://www.figma.com/file/ABC123/") == "ABC123"

    def test_bare_key(self):
        assert extract_file_key("ABC123") == "ABC123"

    def test_bare_key_with_dashes(self):
        assert extract_file_key("abc-def_123") == "abc-def_123"

    def test_invalid_url_raises(self):
        with pytest.raises(ValueError, match="Could not extract"):
            extract_file_key("https://example.com/page")
...
class TestColorConversion:
    ...
    def test_rgb_to_hex_mid_gray(self):
        assert rgb_to_hex(0.5, 0.5, 0.5) == "#808080"

    def test_rgb_to_hex_figma_blue(self):
        # 0.3 * 255 = 76.5, rounds to 76 = 4c
        assert rgb_to_hex(0.4, 0.3, 1.0) == "#664cff"

    def test_get_solid_color_solid(self):
        fills = [{"type": "SOLID", "color": {"r": 1, "g": 0, "b": 0, "a": 1}, "opacity": 1}]
        assert get_solid_color(fills) == "#ff0000"

    def test_get_solid_color_none(self):
        assert get_solid_color(None) is None

    def test_get_solid_color_empty(self):
        assert get_solid_color([]) is None

    def test_get_solid_color_skips_gradient(self):
        fills = [
            {"type": "GRADIENT", "gradientStops": []},
            {"type": "SOLID", "color": {"r": 0, "g": 1, "b": 0, "a": 1}, "opacity": 1},
        ]
        assert get_solid_color(fills) == "#00ff00"

    def test_get_text_color_from_fills(self):
        node = {"fills": [{"type": "SOLID", "color": {"r": 0.5, "g": 0.5, "b": 0.5, "a": 1}, "opacity": 1}]}
        assert get_text_color(node) == "#808080"

    def test_get_text_color_default(self):
        assert get_text_color({}) == "#000000"
...
class TestCanvasDetection:
    def test_get_canvases_single(self, mock_simple_design):
        canvases = get_canvases(mock_simple_design["document"])
        assert len(canvases) == 1
        assert canvases[0]["name"] == "Desktop"

    def test_get_canvases_multi(self, mock_multi_canvas_design):
        canvases = get_canvases(mock_multi_canvas_design["document"])
        assert len(canvases) == 2

    def test_classify_desktop_by_name(self):
        canvas = {"name": "Desktop HD"}
        assert classify_canvas(canvas) == "desktop"

    def test_classify_desktop_by_width(self):
        canvas = {"name": "Page 1", "children": [
            {"absoluteBoundingBox": {"width": 1440, "height": 900}}
        ]}
        assert classify_canvas(canvas) == "desktop"

    def test_classify_mobile_by_name(self):
        canvas = {"name": "Mobile"}
        assert classify_canvas(canvas) == "mobile"

    def test_classify_mobile_by_width(self):
        canvas = {"name": "Page 1", "children": [
            {"absoluteBoundingBox": {"width": 375, "height": 667}}
        ]}
        assert classify_canvas(canvas) == "mobile"

    def test_classify_tablet(self):
        canvas = {"name": "Tablet", "children": [
            {"absoluteBoundingBox": {"width": 768, "height": 1024}}
        ]}
        assert classify_canvas(canvas) == "tablet"

    def test_classify_unknown(self):
        canvas = {"name": "", "children": []}
        assert classify_canvas(canvas) == "unknown"

    def test_get_canvas_dimensions(self, mock_simple_design):
...
class TestWalkNodes:
    def test_walk_simple_design(self, mock_simple_design):
        canvas = mock_simple_design["document"]["children"][0]
        lines = walk_nodes(canvas)
    ...
    def test_walk_includes_positions(self, mock_simple_design):
        canvas = mock_simple_design["document"]["children"][0]
        lines = walk_nodes(canvas)
    ...
    def test_walk_includes_colors(self, mock_simple_design):
        canvas = mock_simple_design["document"]["children"][0]
        lines = walk_nodes(canvas)
        frame_line = next(l for l in lines if "[FRAME]" in l)
        assert "bg:" in frame_line

    def test_walk_includes_text(self, mock_simple_design):
        canvas = mock_simple_design["document"]["children"][0]
        lines = walk_nodes(canvas)
...
class TestFilterFigmaData:
    ...
    def test_filter_strips_plugin_data(self):
        data = {
            "name": "Test",
            "document": {
    ...
    def test_filter_keeps_name_and_modified(self):
        data = {
            "name": "Keep Me",
    ...
    def test_filter_strips_image_data(self, mock_design_with_images):
        filtered = filter_figma_data(mock_design_with_images)
    ...
    def test_filter_preserves_node_structure(self, mock_simple_design):
        filtered = filter_figma_data(mock_simple_design)
        doc = filtered["document"]
    ...
    def test_filter_preserves_text_content(self, mock_simple_design):
        filtered = filter_figma_data(mock_simple_design)
    ...
    def test_filter_size_reduction(self, mock_design_with_images):
        """Filtering should reduce the JSON size."""
        raw = json.dumps(mock_design_with_images)
        filtered_data = filter_figma_data(mock_design_with_images)
...
class TestBuildDesignPrompt:
    def test_build_prompt_includes_design_name(self, mock_simple_design):
        prompt = build_design_prompt(mock_simple_design)
        assert "Simple Design" in prompt

    def test_build_prompt_includes_summary_section(self, mock_simple_design):
        prompt = build_design_prompt(mock_simple_design)
        assert "## Design Tree Summary" in prompt

    def test_build_prompt_includes_filtered_json(self, mock_simple_design):
        prompt = build_design_prompt(mock_simple_design)
        assert "## Filtered Figma JSON" in prompt

    def test_build_prompt_includes_instructions(self, mock_simple_design):
        prompt = build_design_prompt(mock_simple_design)
        assert "## Instructions" in prompt
        assert "Node rendering guide" in prompt

    def test_build_prompt_includes_canvas_label(self, mock_simple_design):
        prompt = build_design_prompt(mock_simple_design)
        assert "Desktop" in prompt

    def test_build_prompt_multi_canvas(self, mock_multi_canvas_design):
        prompt = build_design_prompt(mock_multi_canvas_design)
        assert "Desktop HD" in prompt
        assert "Mobile" in prompt
        assert "responsive" in prompt.lower()

    def test_build_prompt_single_canvas_no_responsive(self, mock_simple_design):
        prompt = build_design_prompt(mock_simple_design)
        assert "responsive" not in prompt.lower()

    def test_build_prompt_no_document_fallback(self):
        prompt = build_design_prompt({"name": "Fallback"})
        assert "could not be fully parsed" in prompt

    def test_build_prompt_empty_document_fallback(self):
        prompt = build_design_prompt({"name": "Empty", "document": {}})
        assert "could not be fully parsed" in prompt
