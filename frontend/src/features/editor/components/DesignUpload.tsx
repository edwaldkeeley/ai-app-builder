...
export interface DesignUploadHandle {
  open: () => void;
}
...
const DesignUpload = forwardRef<DesignUploadHandle, DesignUploadProps>(function DesignUpload(
  { projectId, onUploadComplete, variant = "landing" },
  ref,
) {
  ...
  if (variant === "toolbar") {
    return (
      <>
        ...
        {showModal && (
          <Modal open={showModal} onClose={() => { setShowModal(false); setErrorMsg(null); setSelectedFile(null); setPreviewUrl(null); }}>
    ...
    <div className="bg-surface border border-border rounded-xl p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-text-secondary">Upload an image to generate matching code</p>
        </div>
      </div>
